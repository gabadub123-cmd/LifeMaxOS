    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button
            onClick={() => {
              if (idea.stage === 'killed') return;
              setKillReason('');
              setShowKillModal(true);
            }}
            disabled={idea.stage === 'killed'}
            style={{
              ...footerBtn,
              color: idea.stage === 'killed' ? '#444' : '#FF5252',
              cursor: idea.stage === 'killed' ? 'default' : 'pointer',
            }}
          >
            {idea.stage === 'killed' ? 'Killed' : 'Kill'}
          </button>
          <button
      ...
      {/* Kill confirm modal — capture lesson */}
      {showKillModal && (
        <div
          style={{
        ...
        >
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 14 }}>
              <button onClick={() => setShowKillModal(false)} style={footerBtn}>Cancel</button>
              <button
                onClick={() => {
                  const reason = killReason.trim();
                  if (!reason) return;
                  const note = { ts: new Date().toISOString(), text: `KILLED: ${reason}` };
                  onUpdate({ stage: 'killed', notes: [...notes, note] });
